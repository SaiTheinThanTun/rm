  ...
  tabsetPanel(
    ...
    tabPanel(title = strong("Typology Parameters"),
             column(3,
                    ...
                    sliderInput(inputId="bh_max", label = "number of mosquito bites per human per night (peak season)", value = 15, min=0, max=80,step=1), #change range 0-80, Dan's data
    ...
    tabPanel(title = strong("Focal MDA Indicators"),
             column(3,
                    checkboxInput(inputId="MDAon", label = "switch on MDA", value = TRUE),
             ...
             column(3,
                    ...
                    sliderInput(inputId="avMDA", label = "Added value of focal targetting", value = 70, min=0, max=80,step=10)
             ),
             
             column(3,
                    sliderInput(inputId="prevT2018", label = "Prevalence threshold for defining hotspots in 2018", value = 10, min=0, max=50,step=10),
    ...
    with(as.list(c(state, parameters)),
         {
           ...
           lam <- (1-(effv))*(1-(1-eta)*effIRS*covIRS)*(1-effITN*covITN)*beta*(IC+Tr+rhoa*IA+rhou*IU)/P
