...
ui <- fluidPage(
  tabsetPanel(
    ...
    tabPanel(title = strong("Baseline"),
             column(3,
                    ...
                    sliderInput(inputId="bh_max", label = "number of mosquito bites per human per night (peak season)", value = 13, min=0, max=80,step=1), #change range 0-80, Dan's data
             ...
             column(3,
                    ...
                    sliderInput(inputId="percfail2019", label = "% of cases failing treatment in 2019  ", value = 40, min=0, max=100,step=5),
                    sliderInput(inputId="percfail2020", label = "% of cases failing treatment in 2020 and after  ", value = 50, min=0, max=100,step=5)
             )
    ),
  ...
  tabPanel(title = strong("Reactive case detection"),
            column(4, 
              ...
              sliderInput(inputId="RCDthresh", label = "upper limit on annual incidence per 1000 for RCD", value = 12, min=1, max=12,step=1),
           ...
           column(4, 
            ...
            #          # )
            # ),
            tabPanel(title = strong("Import screen and treat"),
                     column(3,
                            checkboxInput(inputId="MSATon", label = "switch on ISAT", value = FALSE),
                            sliderInput(inputId="MSATscale", label = "years to scale up ISAT ", value = 2, min=.25, max=3, step=.25), 
                            sliderInput(inputId="covMSATi", label = "new coverage of ISAT (%)", value = 80, min=0, max=100,step=10)
                     ),
                     column(3,
                            sliderInput(inputId="MSATsensC", label = "sensitivity ISAT test (clinical) ", value = 95, min=0, max=100,step=5),
                            sliderInput(inputId="MSATsensA", label = "sensitivity ISAT test (micro detectable, asym)", value = 60, min=0, max=100,step=5),
                            sliderInput(inputId="MSATsensU", label = "sensitivity ISAT test (micro undetectable, asym)", value = 0, min=0, max=100,step=5)
                     )
            ),
