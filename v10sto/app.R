...
{
  system("chmod +x stochsim", intern = TRUE)
  a <- read.table(text = system("./stochsim -betascale 1.0 -betascale2 1.5", intern = TRUE),sep="\t")
  #a <- read.table(text = system("./stochsim", intern = TRUE),sep="\t")
...
server <- function(input, output, session) {
  ...
  plotR <- function()
  {
    ...
    # PLOTTING
    par(mfrow=c(1,2))
    plot(times,clinmonth_tot[,1], type = 'l')
    ...
    # 
    # plot(x,y1, type='l',lty=1,col=rgb(0,0,0,alpha=0.1),xlab = "Time",ylab="incidence per 1000 per month",main="Monthly cases per 1000 population",lwd=2)
    ...
    # maxy<-finprev
    # plot(times[(runin:length(prevalence[,1]))],prevalence[(runin:length(prevalence[,1])),1], type='l',lty=1,col=rgb(0,0,0,alpha=0.25),xlab = "Time",ylab="% prevalence",main="Predicted true prevalence",lwd=6)
    # lines(times[(runin:length(prevalence[,1]))],prevalence[(runin:length(prevalence[,1])),2], type='l',lty=1,col=rgb(0,0,1,alpha=0.6),xlab = "Time",ylab="% prevalence",main="Predicted true prevalence",lwd=6)
    # lines(c(2018,2018),c(-maxy,2*maxy),col="dark grey",lty=3,lwd=2)
    # lines(c(2021,2021),c(-maxy,2*maxy),col="dark grey",lty=3,lwd=2)
  }
