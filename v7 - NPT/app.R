...
ui <- fluidPage(
  tabsetPanel(
    ...
    tabPanel(title = strong("Typology Parameters"),
             column(3,
                    sliderInput(inputId="API", label = "baseline API", value = 25, min=1, max=100,step=0.5),
                    sliderInput(inputId="bh", label = "number of mosquito bites per human per day (average)", value = 9, min=0, max=30,step=1),
    ...
    tabPanel(title = strong("New Interventions"),
             ...
             column(4, wellPanel(
               h3("Reactive Case Detection"),
                    ...
                    sliderInput(inputId="RCDscale", label = "years to scale up RCD ", value = 1, min=.25, max=3, step=.25), #.25 timesteps
                    sliderInput(inputId="covRCDi", label = "new coverage of RCD (%)", value = 80, min=0, max=100,step=10),
