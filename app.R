...
ui <- fluidPage(
  tabsetPanel(
    ...
    tabPanel(title = strong("Typology parameters"),
             column(3,
                    numericInput(inputId="R0", label = "# basic reproduction number", value = 2.7, min=0,max=3),
  ...
  fluidRow(plotOutput(outputId = "RAI")),
  br(),
  br(),
  br(),
  br(),
...
server <- function(input, output) {
  output$RAI <- renderPlot({
    ...
    scenario<-c(0,0,0,0,stopyear,input$timei,input$scalei,0)
    RAIout0<-runRAI(scenario)
    scenario<-c(input$EDATon,input$ITNon,input$IRSon,input$RCDon,input$ta2ap2,input$timei,input$scalei,input$CHWon)
