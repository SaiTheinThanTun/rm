...
ui <- fluidPage(
  tabsetPanel(
    ...
    tabPanel(title = strong("Typology Parameters"),
             column(3,
                    sliderInput(inputId="API", label = "baseline API", value = 100, min=1, max=100,step=0.5),
                    sliderInput(inputId="bh", label = "number of mosquito bites per human per day (average)", value = 11, min=0, max=30,step=1),
             ...
             column(3,
                    ...
                    sliderInput(inputId="omega", label = "average duration of immunity (years) ", value = 2, min=0.5, max=10, step=0.5)
             )
    ),
    ...
    tabPanel(title = strong("New Interventions"),
             column(4,
                    ...
                    sliderInput(inputId="EDATscale", label = "years to scale up EDAT ", value = 0.25, min=.25, max=3, step=.25),
    ...
    tabPanel(title = strong("Focal MDA Indicators"),
             column(3,
                    ...
                    sliderInput(inputId="cmda_1", label = "effective population coverage of focal MDA in round 1 ", value = 100, min=0, max=100,step=10),
                    sliderInput(inputId="cmda_2", label = "effective population coverage of focal MDA in round 2 ", value = 100, min=0, max=100,step=10),
                    sliderInput(inputId="cmda_3", label = "effective population coverage of focal MDA in round 3 ", value = 100, min=0, max=100,step=10)
             ),
             column(3,
                    sliderInput(inputId="tm_1", label = "timing of 1st round [2018+ no. of month, 1 means Jan'2018, 13 means Jan'2019]", value = 3, min=1, max=36,step=1),
                    sliderInput(inputId="tm_2", label = "timing of 2nd round [2018+ no. of month]", value = 4, min=2, max=36,step=1),
                    sliderInput(inputId="tm_3", label = "timing of 3rd round [2018+ no. of month]", value = 5, min=3, max=36,step=1),
...
runGMS<-function(initprev, scenario, param) 
{
  ...
  iimm <- c(6,  7,  8,9, 10,12, 14, 15, 16,17, 18,20, 22, 23, 24,25, 26,28, 30, 31, 32,33, 34,36)
  ...
  # mortality
  mortmonth_upp<-100*0.003*clinmonth_tot
  mortmonth_low<-100*0.003*(clinmonth_tot-clinmonth_det)
  
  # % prevalence
  prevalence <- 100*rowSums(out[,iprev])/pop
  immune <- 100*rowSums(out[,iimm])/pop
  ...
  GMSout[,4]<-prevalence
  GMSout[,5]<-immune
  GMSout[,6]<-mortmonth_upp
  GMSout[,7]<-mortmonth_low
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file,{
    ...
    updateSliderInput(session, "omega", value = datavalue()[53])
  })
  ...
  plotR <- function()
  {
    ...
    immune<-cbind(GMSout0[,5],GMSouti[,5])
    mortmonth_upp<-cbind(GMSout0[,6],GMSouti[,6])
    mortmonth_low<-cbind(GMSout0[,7],GMSouti[,7])
    ...
    finimm<-max(immune[(runin:length(immune[,1])),])
    finmort<-max(mortmonth_upp[(runin:length(mortmonth_low[,1])),])
    ...
    plot(x,y1, type='l',lty=1,col=rgb(0,0,0,alpha=0.1),xlab = "Time",ylab="incidence per 1000 per month",main="Monthly cases per 1000 population",ylim=c(0,maxy),lwd=2,cex.lab=enl, cex.axis=0.9*enl, cex.main=1.2*enl, cex.sub=enl)
    ...
    maxy<-finprev
    plot(times[(runin:length(prevalence[,1]))],prevalence[(runin:length(prevalence[,1])),1], type='l',lty=1,col=rgb(0,0,0,alpha=0.25),xlab = "Time",ylab="% prevalence",main="Predicted true prevalence",ylim=c(0,maxy),lwd=6,cex.lab=enl, cex.axis=0.9*enl, cex.main=1.2*enl, cex.sub=enl)
    lines(times[(runin:length(prevalence[,1]))],prevalence[(runin:length(prevalence[,1])),2], type='l',lty=1,col=rgb(0,0,1,alpha=0.6),lwd=6)
    lines(c(2018,2018),c(-maxy,2*maxy),col="dark grey",lty=3,lwd=2)
    lines(c(2021,2021),c(-maxy,2*maxy),col="dark grey",lty=3,lwd=2)
    
    maxy<-finimm
    plot(times[(runin:length(immune[,1]))],immune[(runin:length(immune[,1])),1], type='l',lty=1,col=rgb(0,0,0,alpha=0.25),xlab = "Time",ylab="% immune",main="Predicted herd immunity",ylim=c(0,maxy),lwd=6,cex.lab=enl, cex.axis=0.9*enl, cex.main=1.2*enl, cex.sub=enl)
    lines(times[(runin:length(immune[,1]))],immune[(runin:length(immune[,1])),2], type='l',lty=1,col=rgb(0,0,1,alpha=0.6),lwd=6)
    lines(c(2018,2018),c(-maxy,2*maxy),col="dark grey",lty=3,lwd=2)
    lines(c(2021,2021),c(-maxy,2*maxy),col="dark grey",lty=3,lwd=2)
    
    maxy<-finmort
    x<-times[(runin:length(mortmonth_low[,1]))]
    y1<-mortmonth_low[runin:length(mortmonth_low[,1]),1]
    y2<-mortmonth_upp[runin:length(mortmonth_upp[,1]),1]
    
    plot(x,y1, type='l',lty=1,col=rgb(0,0,0,alpha=0.1),xlab = "Time",ylab="mortality per 100,000 per month",main="Monthly deaths per 100,000 population",ylim=c(0,maxy),lwd=2,cex.lab=enl, cex.axis=0.9*enl, cex.main=1.2*enl, cex.sub=enl)
    lines(x,y2, type='l',lty=1,col=rgb(0,0,0,alpha=0.1),lwd=2)
    
    polygon(c(x,rev(x)),c(y2,rev(y1)),col=rgb(0,0,0,alpha=0.1),border=NA)
    
    y1<-mortmonth_low[runin:length(mortmonth_low[,1]),2]
    y2<-mortmonth_upp[runin:length(mortmonth_upp[,1]),2]
    lines(x,y1, type='l',lty=1,col=rgb(0,0,1,alpha=0.4),lwd=2)
    lines(x,y2, type='l',lty=1,col=rgb(0,0,1,alpha=0.4),lwd=2)
