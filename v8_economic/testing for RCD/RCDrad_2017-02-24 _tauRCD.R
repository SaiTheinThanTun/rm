...
loopValue <- c(5:20,50, 100, 200, 500)
...
for(i in 1:length(loopValue)){ 
  ...
  runGMS<-function(initprev, scenario, param) 
  {
    ...
    state <- c(Y = 0, Cinc_det = 0, Cinc_tot = 0, 
               ...
               S_3 = 0, IC_3 = 0, IA_3 = 0, IU_3 = 0, R_3 = 0, Tr_3 = 0, Sm_3 = 0, Rm_3 = 0, tauRCD=0
    )
      ...
      with(as.list(c(state, parameters)),
           {
             ...
             list(c(dY,dCinc_det,dCinc_tot, 
                    ...
                    dS_3, dIC_3, dIA_3, dIU_3, dR_3, dTr_3, dSm_3, dRm_3, tauRCD
             ))
           }
      ) 
    ...
    GMSout<-matrix(NA,nrow=length(times),ncol=5)
    GMSout[,1]<-times
    GMSout[,2]<-clinmonth_det
    GMSout[,3]<-clinmonth_tot
    GMSout[,4]<-prevalence
    GMSout[,5]<- out[nrow(out),37]
  ...
  GMSouti <- runGMS(initprevR, scenario_i, parametersR)
  
  
  # detected <- sum(GMSouti[,2]) #tail(GMSouti[,2],1) 
  # totalcases <- sum(GMSouti[,3]) #tail(GMSout0[,3],1)
  # total_0 <- sum(GMSout0[,3]) #tail(GMSout0[,2],1)
  #GMSouti[99,2]/GMSout0[99,3]
  result[i, 1] <- GMSouti[nrow(GMSouti),5] #GMSouti[97,3] #detected #/totalcases
...
plot(result3[,3],result3[,1], type='l', xlab="radius size (m)", ylab="tauRCD", main= "tauRCD vs radius")
