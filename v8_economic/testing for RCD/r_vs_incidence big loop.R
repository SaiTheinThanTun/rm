...
bigLoop <- c(0:50)
loopValue <- c(5:20,50, 100, 200, 500, 1000)

big_result <- list()
result <- matrix(NA, length(loopValue), 2) 
for(j in 1:length(bigLoop)){
for(i in 1:length(loopValue)){ 
  ...
  big_result[[j]] <- cbind(result,loopValue)
}
}
...
#plot(result3[,3],result3[,1], type='l', xlab="radius size (m)", ylab="incidence")
